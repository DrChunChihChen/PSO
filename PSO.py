...
def invisible_walls(X, boundaries):
    ...
    for j in range (X.shape[1]):
        ...
        X[particles_crossed] = np.random.rand (particles_crossed.shape[0], X.shape[1]) * (
                    boundaries[j, 1] - boundaries[j, 0]) + boundaries[j, 0]
    return X
...
def runPSO(X, pbest_particles, pbest_fitness, gbest_particle, gbest_fitness, boundaries, V, I, M, N, w, c1, c2, dt,
           boundary_condition):
    """
    ...
    for i in range (I):
        ...
        V = w * V + c1 * np.random.rand (M, N) * (pbest_particles - X) + c2 * np.random.rand (M, N) * (
                    gbest_particle - X)
...
if __name__ == "__main__":
    ...
    # Run iterations
    gbest_particle, gbest_fitness = runPSO (X, pbest_particles, pbest_fitness, gbest_particle, gbest_fitness,
                                            boundaries, V, I=1000, M=1000, N=2, w=0.1, c1=0.1, c2=0.1, dt=0.1,
                                            boundary_condition="absorbing_walls")
